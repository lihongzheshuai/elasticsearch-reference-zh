...
索引操作会根据路由规则直接分配到主分块上（参加上面路由章节介绍）然后在保存该分块的节点上执行操作。当主分块完成操作后，如果需要，该更新会分发到可用的副本上。
...
当使用索引接口更新的文档的时候，即使文档没有任何变化，文档的版本也会更新。如果在使用_update api时不想要此效果，可将detect_noop的值设置为true。该参数在索引api中不可用，因为索引api不获取旧的数据因此无法与新数据进行比较。

何时不可接受无变化更新没有一个硬性的快速的规则。这是由许多因素构成的，如你的数据多久会发送一次无变化的更新操作，以及Elasticsearch在接收更新的分块上每秒有多少请求。

## 超时

分配执行索引操作的主分块在索引操作完成后可能不可用。一些可能原因是当前主分块正在从网管恢复或者正在进行重分配。默认情况下，索引操作会最多等待1分钟，等待主分块变成可用态，超时则操作失败返回错误信息。timeout参数可用来指定等待时长。下面的例子为设置成5分钟超时：
