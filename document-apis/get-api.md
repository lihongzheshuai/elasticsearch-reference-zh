...
获取操作会通过hash计算分配到指定id的分块上。然后该操作会重分配到该id分块的其他副本上然后返回结果。副本是指主分块以及属于分块id组的副本。这意味着，副本越多，GET操作的扩展性（scaling）越强。

## 版本支持

仅当当前版本等于你指定值的时候，你才可以使用version参数来获取文档。除了版本类型是FORCE的情况，其他所有版本类型的行为都相同，FORCE类型总是会返回文档。（译者注：这里主要指指定版本与当前版本不匹配的情况，除了FORCE类型，其他情况都会返回异常。）

在内部，Elasticsearch会将旧版本标记为删除状态然后添加全新的文档。尽管你无法再访问旧版本的文档，但是它不会立即消失。当你继续索引更多数据的时候，Elasticsearch会在后台清理删除的文档。
